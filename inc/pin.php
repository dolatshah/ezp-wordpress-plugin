...
if( isset($_GET['list']) ){
	...
	if(  isset($can_display_result) && isset($_GET['id']) && $_GET['id']!='' ){
		...
		$client_id 		= 0;
		$sql_pin = $mysqli->query("SELECT * FROM wp_ezpay_charge where client_id='$client_id' and type='pin' $cond ORDER BY id DESC ") or die($mysqli->error);
	...
	if( isset($_POST['submit']) && ( isset($_GET['MTN']) || isset($_GET['MCI']) || isset($_GET['RTL']) ) ){
		...
		elseif( !in_array($amount, $valid_amount)) {
			...
			$count=1;
			$sql= $mysqli->query("INSERT INTO wp_ezpay_charge (client_id,type, platform ,mobile, product_type, amount,date) VALUES ('$client_id', 'pin', 'client_area' , '$mobile', '$operator', '$amount', '$date') " );
			$status = 'unpaid';
			$ref_code  = $res_code = '';

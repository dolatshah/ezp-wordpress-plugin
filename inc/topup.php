...
if( isset($_GET['list']) ){
	...
	if(isset($_GET['ezpay_token']) && $_GET['ezpay_token']!=''){
		...
		if($decrypted['status']){
			...
			if($status == 'paid'){
				$mysqli->query("update wp_ezpay_charge set status='$status',ref_code='$ref_code',pay_date='$date',pay_result='$pay_result' where id='$order_id' and status='unpaid' ");
			}else{
	...
	if( isset($can_display_result) && isset($_GET['id']) && $_GET['id']!=''){
		...
		$client_id 		= 0;
		$sql_charge 	= $mysqli->query("SELECT * FROM wp_ezpay_charge where client_id='$client_id' and type='topup' $cond ORDER BY id DESC ") or die($mysqli->error);
	...
	if( isset($_POST['submit']) && ( isset($_GET['MTN']) || isset($_GET['MCI']) || isset($_GET['RTL']) ) ){
		...
		elseif( isset($custom_amount) && ($amount<500 ||  $amount>200000) ){
		...
		elseif( $charge_type!= "normal" && $charge_type!= "amazing" && $charge_type!= "mnp" && $charge_type!= "permanent" ){
			...
			$sql= $mysqli->query("INSERT INTO wp_ezpay_charge (client_id,type, platform ,mobile, product_type, amount,date) VALUES ('$client_id', 'topup', 'client_area' , '$mobile', '$operator', '$amount', '$date') " );

...
if( isset($_GET['list']) ){
	...
	if(isset($_GET['ezpay_token']) && $_GET['ezpay_token']!=''){
		...
		if($decrypted['status']){
			...
			if($status == 'paid'){
				$mysqli->query("update wp_ezpay_charge set status='$status',ref_code='$refcode',pay_date='$date',pay_result='$pay_result' where id='$order_id' and status='unpaid' ");
			}else{
				$mysqli->query("update wp_ezpay_charge set status='$status',pay_result='$pay_result' where id='$order_id' ");
			}
		}
	}
	...
	if( isset($can_display_result) && isset($_GET['id']) && $_GET['id']!='' ){
		...
		$client_id 	= 0;
		$sql_charge = $mysqli->query("SELECT * FROM wp_ezpay_charge where client_id='$client_id' and type='internet' $cond ORDER BY id DESC ") or die($mysqli->error);
	...
	if(isset($_GET['MTN']) || isset($_GET['MCI']) || isset($_GET['RTL'])){
		...
		if( (isset($_GET['MTN']) || isset($_GET['MCI']) || isset($_GET['RTL']) ) && !isset($_GET['sim']) ){
		...
		elseif( (isset($_GET['MTN']) || isset($_GET['MCI']) || isset($_GET['RTL'])) && isset($_GET['sim']) && ($_GET['sim']=='credit' || $_GET['sim']=='permanent' ) ){
			...
			$result = RequestJson_Last('get_products',array(), 'ezpay');
				...
				if($result['code']==1){
					...
					if( !isset($pro['internet']) ){
						...
						if( !isset($internet_package[$operator][$sim_type][$in_type][$pid]) ){
							...
							$sql= $mysqli->query("INSERT INTO wp_ezpay_charge (client_id, type, platform ,mobile, product_type, amount,date) VALUES ('$client_id', 'internet', 'client_area' , '$mobile', '$operator', '$amount', '$date') " );
							$status = 'unpaid';
							$ref_code  = $res_code = '';
							...
							if($sql){
								...
								$result = RequestJson_Last('invoice',$param, 'ezpay');
								
								if( isset($result) && $result!=false ){
									if($result['code']==1){
										$trans_id = $result['trans_id'];
										$url = "https://ezpay.ir/pay.php?tid={$trans_id}";
										if(headers_sent()){
								...
								$mysqli->query("update wp_ezpay_charge set status='$status',res_code='$res_code',ref_code='$ref_code',payment_type='$payment_type' WHERE id='$charge_id' ");
							}
						}
					}
				}else{
